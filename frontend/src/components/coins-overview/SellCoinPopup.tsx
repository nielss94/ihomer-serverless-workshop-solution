...
export function SellCoinPopup({ trigger, coinId }: Props) {
    const coins = useCoinsStore((state) => state.coins);
    const portfolio = usePortfolioStore((state) => state.portfolio);
    const [coin, setCoin] = useState<CryptoCoin | undefined>(undefined);
    const [ownedCoin, setOwnedCoin] = useState<OwnedCryptoCoin | undefined>(
        undefined
    );
    ...
    useEffect(() => {
        setOwnedCoin(portfolio.find((coin) => coin.id === coinId));
    }, [portfolio, coinId]);
        ...
        <Popup modal nested trigger={trigger} position="right center">
            {(onClose: any) => (
                <div className="modal p-4">
                    <div className="flex">
                        <div className="flex-1 text-2xl">Sell {coin?.name}</div>
